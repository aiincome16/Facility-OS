...
function getIconSvg(iconName) {
    const icons = {
        ...
        key: `
            <svg viewBox="0 0 24 24" aria-hidden="true">
                ...
                <path d="M12 12h8M17 12v3M20 12v2"></path>
            </svg>
        `,
    ...
    return icons[normalizeText(iconName)] ?? icons.more;
}
...
export function renderModuleCard({
    ...
    const normalizedTitle =
        normalizeText(title) || "Funktion";

    const normalizedDescription =
        normalizeText(description);

    const normalizedTone =
        normalizeText(tone) || "blue";
    ...
    if (normalizedRoute) {
        attributes.push(
            `data-route="${escapeHtml(normalizedRoute)}"`
        );
    }

    if (normalizedAction) {
        attributes.push(
            `data-action="${escapeHtml(normalizedAction)}"`
        );
    }
